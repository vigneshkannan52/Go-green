...
function cdi_import_files() {
    return array(
        array(
            'import_file_name'             => __('NGO Charity Connection Free','cdi'),
            'categories'                   =>  array( 'Free' , 'Homepage' ),
            'local_import_file'            => CDI_PLUGIN_DIR_PATH . '/themes/ngo-charity-connection/inc/content.xml',
            'local_import_customizer_file' => CDI_PLUGIN_DIR_PATH . '/themes/ngo-charity-connection/inc/options.dat',
...
function cdi_after_import_setup( $selected_import ) {
    ...
    switch ( $import_file_name ) {
        ...
        case 'NGO Charity Connection Free':
             $front_page_id = get_page_by_title( 'NGO Charity Connection Home' );
            break;
